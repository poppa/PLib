...
/**
 * A cointainer class that represents a "fake" DomNode.
...
class XMLNode
{
   ...
   */
	 ...
	 * @return XMLNode
	 */
	public function AddNode($name, $value=null, array $attributes=array())
	{
   ...
   * @throws PlibException
	 ...
	 */
	public function AddDomNode($node)
	{
		...
		if (!($node instanceof DOMElement)) {
			throw new PLibException(
				"The node given to <strong>XMLNode::AddDomNode()</strong> is not " .
				"a DOMNode object"
			);
		}
	 ...
	 * @return XMLNode
	*/
	public function AddCDATANode($name, $value)
	{
    $n = $this->node ? new self($this->owner) : $this;
   ...
   *
   * @see XMLNode::AddCDATANode()
   * @param string $name
   * @param string $value
   * @return XMLNode
   */
  public function AddNodeAsCDATA($name, $value)
  {
    return $this->AddCDATANode($name, $value);
  }
  ...
  public function Replace($to)
  {
	 ...
	 * @param string $which
    */
	public function RemoveNodeByName($which)
	{
...
class XMLDocument extends XMLNode
{
  ...
  public function Encoding()
  {
	 ...
	 *
	 * @see XMLDocument::Render()
	*/
	public function Finalize($html=false, $format=true)
	{
		$this->Render($html, $format);
	}
}
 ...
 * @return string
 */
function safe_xml($text, $utf8Encode=false)
{
	$out = '';
	$pos = 0;
	...
	$out = Ent::ReplaceHtml($out . htmlspecialchars($text, ENT_NOQUOTES));
	return $utf8Encode ? utf8_encode($out) : $out;
}
?>